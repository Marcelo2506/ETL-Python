...
class User(BaseModel):
    ...
    Link_clicks: Optional[int] = Field(
        None, description="Número de clicks no link")
    Impressions: int = Field(0, description="Número de Impressões no anúncio")
    Conversions: Optional[int] = Field(
        None, description="Número de Conversões Registradas")
